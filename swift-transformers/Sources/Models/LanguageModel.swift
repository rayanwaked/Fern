...
public class LanguageModel {
    ...
    let full_sequence_length = "full_sequence_length"
...
public extension LanguageModel {
    ...
    func predictNextTokenScores(_ tokens: InputTokens, config: GenerationConfig) -> any MLShapedArrayProtocol {
        ...
        // Add the full_sequence_length feature
        let fullSequenceLengthArray = try? MLMultiArray(shape: [1], dataType: .int32)
        fullSequenceLengthArray?[0] = NSNumber(value: inputTokens.count)
        inputDictionary[full_sequence_length] = MLFeatureValue(multiArray: fullSequenceLengthArray!)
        
        let input = try! MLDictionaryFeatureProvider(dictionary: inputDictionary)
        
        let output = try! model.prediction(from: input)
        
        // Print all output feature names and their types
        ...
        // Retrieve logits
        guard let logitsFeature = output.featureValue(for: "logits") else {
            fatalError("Logits not found in model output")
        }
        
        // Convert Float16 to Float
        let logitsArray = logitsFeature.multiArrayValue!
        let float16Pointer = logitsArray.dataPointer.bindMemory(to: UInt16.self, capacity: logitsArray.count)
        
        var float32Array = [Float](repeating: 0, count: logitsArray.count)
        for i in 0..<logitsArray.count {
            float32Array[i] = Float(half: float16Pointer[i])
        }
        
        // Create a MLShapedArray from the converted float32 array
        let float32LogitsArray = MLShapedArray<Float>(scalars: float32Array, shape: logitsArray.shape.map { $0.intValue })
        
        print("Logits shape: \(float32LogitsArray.shape)") // Print the shape for debugging
        
        // Assuming logits shape is (1, 1, 50257), access the correct slice
        let nextTokenScores = float32LogitsArray[0, maxTokens - 1]
        return nextTokenScores
    }
}

// Extension to convert Float16 to Float
extension Float {
    init(half: UInt16) {
        var f16 = half
        self = withUnsafePointer(to: &f16) {
            $0.withMemoryRebound(to: UInt32.self, capacity: 1) {
                Float(bitPattern: $0.pointee << 16)
            }
        }
    }
}
